...
Si no se utiliza _Firebase Auth_, el camino natural es implementar directamente el flujo de [Authorization Code + PKCE](#authorization-code-flow--pkce) + [OpenID Connect (OIDC)](#openid-connect-oidc) con los proveedores de identidad (Google, Apple, Facebook, correo/sms propios, etc.), lo que implica un mayor nivel de complejidad y responsabilidad operativa.
